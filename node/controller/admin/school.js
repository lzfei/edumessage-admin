...
class School {
    ...
    async updateSchoolDetail(req, res, next) {
        const form = req.body;
        try {
            await SchoolDetailModel.findOneAndUpdate({ _id: form._id }, { $set: { total: form.total, egalperson: form.egalperson, mail: form.mail, phone: form.phone, position: form.position, desc: form.desc, items: form.items, image: form.image } });
            res.send({
                status: 1,
                type: 'SUCCESS',
                message: '编辑成功'
