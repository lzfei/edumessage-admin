...
class Teacher {
    ...
    async updateTeacherDetail(req, res, next) {
        const form = req.body;
        try {
            await TeacherDetailModel.findOneAndUpdate({ _id: form._id }, { $set: { date: form.date, egalperson: form.egalperson, mail: form.mail, phone: form.phone, position: form.position, desc: form.desc, items: form.items, image: form.image } });
            res.send({
                status: 1,
                type: 'SUCCESS',
                message: '编辑成功'
